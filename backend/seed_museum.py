...
200 museums × 5 shows each = 1000 records
...
def build_museum_doc(museum: dict, owner_email: str, index: int) -> dict:
    ...
    return {
        ...
        "category": mtype,
        "description": f"A premier {mtype} museum in {museum['city']} showcasing the rich heritage and culture of {museum['state']}.",
...
async def seed():
    ...
    for i, owner in enumerate(OWNERS):
        count = len([m for j, m in enumerate(INDIAN_MUSEUMS) if j % len(OWNERS) == i])
        print(f"   {owner}: {count} museums")

    # Create indexes for fast queries
    await db.museums.create_index("owner_email")
    ...
    client.close()
    print("\n🎉 Done! Your museum dataset is ready.")
